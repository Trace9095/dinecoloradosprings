...
import { Beer, SlidersHorizontal } from 'lucide-react'
...
export default function BreweriesPage() {
  ...
  useEffect(() => {
    fetch('/api/venues?category=brewery')
      .then((r) => r.json())
      .then((data: Venue[]) => {
        setAllVenues(data)
        setLoading(false)
      ...
      <div className="bg-[#161B22] border-b border-[#30363D] py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 mb-3">
            <Beer className="h-6 w-6 text-[#D4A853]" />
            <h1 className="text-3xl font-bold text-[#E6EDF3]">
              Craft Breweries in Colorado Springs
            </h1>
          </div>
          <p className="text-[#8B949E] mb-6 max-w-2xl">
            From the legendary Bristol Brewing at Ivywild School to nationally
            recognized wild ale producers — Colorado Springs has a world-class
            craft beer scene.
          </p>
          <SearchBar
            onSearch={setSearch}
            placeholder="Search breweries, neighborhoods..."
          />
        </div>
      </div>
